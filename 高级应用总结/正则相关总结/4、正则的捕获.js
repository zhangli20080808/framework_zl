...
* 捕获:把正则匹配到的内容捕获到:捕获到的结果是一个数组,数组第一项是当前正则匹配捕获的内容
* index:捕获的开始索引位置
* input:捕获的原始字符串
*
* 修饰符 总结
...
*
*  每一次执行exec只能捕获到一个匹配的,想把所有匹配的都捕获到,至少要执行多次
*  但是一般情况下，我们不管执行多少次，每一次捕获的内容都是和第一次一模一样，后面的2017是捕获不到的
* */


//->"正则捕获的懒惰性"
//    var str = "zhufeng2016peixun2017";z
//    var reg = /\d+/;
//    console.log(reg.exec(str));//->["2016", index: 7, input: "zhufeng2016peixun2017"]
//    console.log(reg.exec(str));//->["2016"...] //捕获到的还是2016 懒惰的特性

/*
* 为啥会出现懒惰性?
* reg.lastIndex:正则每一次捕获的时候,在字符串中开始查找的索引
* 正则每一次捕获结束后,默认的没有把lastIndex值进行修改，lastIndex一直是零,导致第二次捕获还是从字符串的起始位置开始查找,
* 导致每一次捕获的都是第一个和正则匹配的
* */
...
//->有n个的匹配的就需要执行n次exec这个方法,比较的麻烦,生活如此美好，何必这么麻烦?
// ->字符串中提供了一个叫做match的方法,这个方法可以一次执行把所有匹配的捕获到

// var str = "zhufeng2016peixun2017";
// var reg = /\d+?/g;//->不管用哪个方法,g是不能少的  加了?取消了贪婪性 只匹配一个
// console.log(reg.exec(str)) //只得到第一个
// console.log(str.match(reg));//->["2"，"0"，"1"，...]

// 在正则捕获的时候 我们加 g 可以取消捕获时候的懒惰性，
// 原理: 正则有一个lastIndex 属性 代表下一次正则捕获开始的索引 但是默认这个值永远是0 也就是 不管执行多少次
// 都是从头开始捕获 所以获取的结果都是同一个结果，加了g之后 每一次exec执行完成之后 我们的 lastIndex就等于当前
// 捕获内容的后一个索引 下一次从这之后继续查找捕获，这样就可以一次次执行，把想要的内容捕获到

//->但是match也有自己的局限性?
// 如果正则中出现分组,而且需要执行多次exec才能全部捕获的,使用match不能把分组的内容捕获到
...
//     res = reg.exec(str)
// }
// console.log(arr) //捕获到所有结果 [ '2', '0', '1',....]

// const arg = str.match(reg)
// console.log(arg)  [ '2', '0', '1',....]
